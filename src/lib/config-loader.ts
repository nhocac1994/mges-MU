...
export async function loadConfigFromAPI(): Promise<any> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`Config API returned status ${response.status}:`, response.statusText);
    ...
    if (result.success && result.data) {
      ...
      console.warn('Config API response missing data:', result);
      throw new Error('Invalid API response format');
    }
  } catch (error) {
    console.error('Error loading config from API:', error);
    console.warn('Falling back to static config file');
  }
...
export async function loadDownloadUrlsFromAPI(): Promise<string[]> {
  try {
    ...
    console.error('Error loading download URLs from API:', error);
  }
