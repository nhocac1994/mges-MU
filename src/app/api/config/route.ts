...
export async function GET() {
  try {
    ...
    if (response.ok) {
      ...
      console.log('Backend C# response:', result);
      ...
      if (result.success && result.data && result.data.content) {
        ...
        if (typeof configData === 'string') {
          try {
            ...
            console.error('Failed to parse config JSON from backend:', e);
        ...
        console.warn('Backend response missing content:', result);
        throw new Error('Backend response missing content');
      }
    } else {
      console.warn(`Backend C# returned status ${response.status}:`, response.statusText);
    ...
    if (error.name === 'AbortError' || error.message?.includes('fetch')) {
      console.warn('Backend C# không khả dụng hoặc timeout. Sử dụng config tĩnh.');
    }
